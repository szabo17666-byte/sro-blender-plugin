bl_info = {
    ...
    "author": "szabo176 & Gemini",
    "version": (6, 7, 1),
    "blender": (4, 5, 0),
    "location": "3D View > Sidebar > SRO Project",
    "description": "Import Silkroad map with terrain, textures, lightmaps and optimized water from game files.",
...
def get_water_material(sro_root, wtype, wwave):
    ...
    if wave_fn and base_img_node:
        ...
        if wave_img:
            ...
            links.new(base_img_node.outputs['Color'], add.inputs[1])
            links.new(t_wave.outputs['Color'], add.inputs[2])
...
def get_pair_material(sro_root, base_tid, layer_tid, lightmap_img):
    ...
    if lightmap_img:
        lm_tex = nodes.new("ShaderNodeTexImage"); lm_tex.location = Vector((0, -400))
        ...
        lm_mix = nodes.new("ShaderNodeMixRGB"); lm_mix.location = Vector((450, 0))
...
def paint_region(obj, tex_ids_flat, sro_root, lightmap_img):
    me = obj.data
    me.materials.clear()
    ...
    for (a, b), faces in mat_faces.items():
        mat = get_pair_material(sro_root, a, b, lightmap_img)
        if not mat: continue
...
class SRO_OT_ExecuteImport(Operator):
    ...
    def execute(self, ctx):
        ...
        print(f"  Paint Terrain: {'ON' if p.import_textures else 'OFF'}, Import Lightmaps: {'ON' if p.import_lightmaps else 'OFF'}, Import Water: {'ON' if p.import_water else 'OFF'}")
        ...
        for i, (rx, rz) in enumerate(tiles, 1):
            ...
            try:
                H, Tex, W_Data = read_mapm_data(mpath)
                ...
                if p.import_textures and TERRAIN_DDJ:
                    ...
                    paint_region(obj, tex_flat, sro_root, lightmap_image)
