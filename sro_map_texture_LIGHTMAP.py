bl_info = {
    ...
    "description": "Import Silkroad map with correct terrain texture blending and lightmaps from .m, .t and tile2d.ifo.",
...
def parse_mfo(mfo_path: str):
    ...
    print(f"[INFO] MFO OK: {len(regs)} active regions found in mapinfo.mfo.")
    return regs
...
def load_ddj_image(map_root: str, rel_path: str):
    ...
    ddj_path = os.path.join(map_root, rel_path)
    if not os.path.exists(ddj_path):
        ddj_path = os.path.join(map_root, "tile2d", os.path.basename(rel_path))
        if not os.path.exists(ddj_path):
...
def get_pair_material(map_root, base_tid, layer_tid, lightmap_img):
    a = min(base_tid, layer_tid)
    b = max(base_tid, layer_tid)
    lm_name_part = f"_LM" if lightmap_img else ""
    key = (a, b, lightmap_img.name if lightmap_img else None)
    ...
    mat_name = f"MAT_{a:03d}_{b:03d}{lm_name_part}"
    ...
    t_base = nodes.new("ShaderNodeTexImage"); t_base.location = (-450, 150)
    ...
    if a != b and layer_rel:
        ...
        if layer_img:
            t_layer = nodes.new("ShaderNodeTexImage"); t_layer.location = (-450, -100)
            t_layer.image = layer_img
            links.new(mapping.outputs['Vector'], t_layer.inputs['Vector'])
            
            vattr = nodes.new("ShaderNodeAttribute"); vattr.location = (-450, -350)
            vattr.attribute_name = "Blend"
            
            mix = nodes.new("ShaderNodeMixRGB"); mix.location = (200, 0)
            mix.blend_type = 'MIX'
            links.new(t_base.outputs['Color'], mix.inputs[1])
            links.new(t_layer.outputs['Color'], mix.inputs[2])
            links.new(vattr.outputs['Color'], mix.inputs[0])
            final_color_node = mix

    if lightmap_img:
        lm_tex = nodes.new("ShaderNodeTexImage"); lm_tex.location = (0, -400)
        ...
        lm_mix = nodes.new("ShaderNodeMixRGB"); lm_mix.location = (450, 0)
        lm_mix.blend_type = 'MULTIPLY'
        lm_mix.inputs[0].default_value = 1.0
        links.new(final_color_node.outputs[0], lm_mix.inputs[1])
        links.new(lm_tex.outputs['Color'], lm_mix.inputs[2])
        final_color_node = lm_mix

    links.new(final_color_node.outputs[0], bsdf.inputs['Base Color'])
    MATERIAL_CACHE[key] = mat
    return mat

def paint_region(obj, tex_ids_flat, map_root, lightmap_img):
    me = obj.data
    me.materials.clear()
    ...
    for (a, b), faces in mat_faces.items():
        mat = get_pair_material(map_root, a, b, lightmap_img)
        if not mat: continue
...
class SRO_OT_ParseData(Operator):
    ...
    def execute(self, ctx):
        ...
        try:
            ...
            global MATERIAL_CACHE, TEXTURE_CACHE, LIGHTMAP_CACHE
...
class SRO_OT_ExecuteImport(Operator):
    ...
    def execute(self, ctx):
        ...
        print(f"  Paint Terrain: {'ON' if p.import_textures else 'OFF'}, Import Lightmaps: {'ON' if p.import_lightmaps else 'OFF'}")
        ...
        for i, (rx, rz) in enumerate(tiles, 1):
            ...
            try:
                ...
                if p.import_textures and TERRAIN_DDJ:
                    ...
                    paint_region(obj, tex_flat, map_root, lightmap_image)
...
class SRO_PT_Project(Panel):
    ...
    def draw(self, ctx):
        ...
        if p.is_root_valid and p.is_data_parsed:
            ...
            if p.import_textures:
                row = col.row()
                row.separator()
                row.prop(p, "import_lightmaps")
            
            layout.separator()
            op = layout.operator(SRO_OT_ExecuteImport.bl_idname, icon='PLAY', text="Import Map")
...
def unregister():
    ...
    for c in reversed(CLASSES):
        try:
            ...
            pass # Ignore 'class still has properties' error on reload
